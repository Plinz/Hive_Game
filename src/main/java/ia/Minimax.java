...
public class Minimax {
    ...
    public List<Minimax> getChildrenWithHeuristics() {
        ...
        if ((core.getTurn() == 6 || core.getTurn() == 7) && !core.isQueenOnBoard()) {
            Piece queen = null;
            for (Piece piece : players[currentPlayer].getInventory()){
            ...
            for (CoordGene<Integer> possibleAdd : core.getPossibleAdd(AIPlayer)){
                possibleMovements.add(Notation.getMoveNotation(board, queen, possibleAdd));
                possibleUnplay.add(Notation.getInverseMoveNotation(board, queen));
            }
        } else {
